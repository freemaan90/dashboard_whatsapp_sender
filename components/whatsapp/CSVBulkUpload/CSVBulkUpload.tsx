...
import { interpolateMessage } from '@/lib/parseExcelPhones';
import { parseCSVPhones } from '@/lib/parseCSVPhones';
import { normalizeArgPhone } from '@/lib/validatePhone';
import { sendMessage } from '@/app/actions/sendMessage';
import type { BulkSendState } from '@/components/whatsapp/BulkUpload/BulkUpload';
import styles from '../BulkUpload/BulkUpload.module.css';
...
export function CSVBulkUpload({ onPhonesLoaded, disabled, sessionId, message, onStatusChange, registerSend }: CSVBulkUploadProps) {
  ...
  async function processFile(file: File) {
    ...
    if (ext !== 'csv') {
      setError('Formato de archivo no compatible. Solo se aceptan archivos .csv.');
      setFileName('');
      setValidPhones([]);
    ...
    try {
      const { rows } = await parseCSVPhones(file);

      if (rows.length === 0) {
        setError('No se encontraron números en el archivo.');
        setValidPhones([]);
        validPhonesRef.current = [];
        setInvalidPhones([]);
        onPhonesLoaded([]);
      ...
      setError('Error al leer el archivo. Verifique que sea un CSV válido.');
      setValidPhones([]);
      validPhonesRef.current = [];
      setInvalidPhones([]);
      onPhonesLoaded([]);
  ...
  return (
    <div className={styles.container}>
      <div className={styles.sectionHeader}>
        <span className={styles.sectionTitle}>Envío masivo CSV</span>
        <span className={styles.sectionHint}>Cargá un archivo CSV (.csv) con los números en la primera columna</span>
      </div>
      ...
      <div
        ...
        aria-label="Cargar archivo CSV con números de teléfono"
      >
        <input
          ref={inputRef}
          type="file"
        ...
        {fileName ? (
          ...
          <>
            ...
            <span>Arrastra un archivo .csv o haz clic para seleccionar</span>
          </>
        )}
      </div>
